#!/usr/bin/env bash
#
# run_final_eval_orpo.sh
#
# Server B launcher: full engagement-aware eval on
# ORPO 50k (single-stage, no ref model, λ=0.1).
# Reads runs/orpo_50k/merged.
#
...
#
# Outputs:
#   evaluation_results/eval_engaged_orpo_full.csv
#   evaluation_results/eval_engaged_orpo_full.json
#   evaluation_results/final_eval_summary_orpo.json   ← headline JSON
#                                                       (matches name
#                                                        referenced in
#                                                        run_final_eval_baseline_sft.sh)
#
# Usage:
#     bash scripts/run_final_eval_orpo.sh
#     BATCH_SIZE=8 bash scripts/run_final_eval_orpo.sh
...
# run_orpo_50k.sh used oneRec/qwen3_soft_switch.jinja2 — but resolve_template
# falls back automatically if the model/ copy is missing. Keep model/ for
# parity with the other final-eval scripts.
TEMPLATE="model/qwen3_soft_switch.jinja2"

# Trained checkpoint produced by scripts/run_orpo_50k.sh
RUN="runs/orpo_50k"
MODEL="$RUN/merged"
...
if [ ! -d "$MODEL" ]; then
    echo ">>> [error] ORPO 50k merged checkpoint missing at $MODEL"
    echo "    Training likely still in flight, or run scripts/run_orpo_50k.sh first."
    exit 1
fi

# ============================================================
# STEP 1 — ORPO 50k × engagement-aware on full v1 valid (~14k)
# ============================================================
echo ""
echo "============================================================"
echo ">>> ORPO 50k × engagement-aware on full v1 valid"
echo ">>> model: $MODEL"
echo ">>> rows: ~14,000 (full)  batch=$BATCH_SIZE"
echo "============================================================"

CSV="$EVAL_OUT/eval_engaged_orpo_full.csv"
JSON="$EVAL_OUT/eval_engaged_orpo_full.json"
LOG="$EVAL_LOGS/eval_engaged_orpo_full.log"
...
echo ""
echo "############################################################"
echo "# ORPO FINAL EVAL COMPLETE  (total $(fmt_min $START $END))"
echo "############################################################"
echo ""
echo "Outputs:"
echo "  $CSV"
...
# final_eval_summary_orpo.json (single-arm: engaged only).
# ============================================================
COMBINED_JSON="$EVAL_OUT/final_eval_summary_orpo.json"
...
echo ""
echo ">>> When server A (DPO+anchor 50k) and server C (baseline + SFT-50k) finish,"
